...
function App() {
  ...
  const LoadBlockchainData=async()=>{
    ...
    setAcc(account)
    if(networkData1){
      const Edublocks_Contract=new web3.eth.Contract(EdublocksABI.abi,networkData1.address)
      setContract(Edublocks_Contract)
      if(networkData2){
        const Token_Contract=new web3.eth.Contract(TokenABI.abi,networkData2.address)
        setT_contract(Token_Contract)
        if(networkData3){
          const TokenSale_Contract=new web3.eth.Contract(TokenSaleABI.abi,networkData3.address)
  ...
  return (
    <div className="App">
      ...
      <Switch>
        ...
        <Route path="/student" render={(props)=>(<Student {...props} ts_contract={ts_contract}
        address={acc} contract={contract} t_contract={t_contract}/>)}></Route>
        <Route path="/educator" render={(props)=>(<Educator {...props} ts_contract={ts_contract}
        address={acc} contract={contract} t_contract={t_contract}/>)}></Route>
        <Route path="/course" render={(props)=>(<Course {...props} address={acc} contract={contract}/>)}></Route>
        <Route path="/" render={(props)=>(<Home {...props} ts_contract={ts_contract}
        address={acc} contract={contract} t_contract={t_contract} error={err}/>)} exact></Route>
