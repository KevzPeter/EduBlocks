...
export const Educator = ({ address, contract, t_contract, ts_contract }) => {
    ...
    const ShowSubmissions = () => {
        return (
            <div>
                {submissions.map((el, idx) => <SubmissionCard key={idx} address={address}
                    std_name={el.std_name} marks={el.marks}
                    course_name={el.course_name} std_id={el.std_id}
                    course_id={el.course_id} contract={contract}
                    tx={el.transaction_hash} content={el.content}
                    student_address={el.address} ts_contract={ts_contract} />)}
            </div>
        )
    }
                            ...
                            <Tab.Content>
                                <Tab.Pane eventKey="first">
                                    <div className="info-panel">
                                        ...
                                        <div className="right">
                                            <Button className="btn-primary my-3" variant="dark" onClick={handleShow}>Add Course</Button>
                                        </div>
                                    </div>
